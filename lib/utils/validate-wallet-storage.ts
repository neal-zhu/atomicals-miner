...
export const resolveWalletPath = (n: number=0): string => {
  let basePath = '.';
...
export const validateWalletStorage = async (n :number=0): Promise<IValidatedWalletInfo> => {
  try {
    const path = resolveWalletPath(n)
    console.log('walletPath', path);
    const wallet: any = await jsonFileReader(path);
    if (!wallet.phrase) {
      console.log(`phrase field not found in ${path}`);
      throw new Error(`phrase field not found in ${path}`)
    }
