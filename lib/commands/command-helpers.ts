...
export const prepareCommitRevealConfig = (opType: 'nft' | 'ft' | 'dft' | 'dmt' | 'sl' | 'x' | 'y' | 'mod' | 'evt' | 'dat', keypair: KeyPairInfo, atomicalsPayload: AtomicalsPayload, log = true) => {
    ...
    //console.log("payload", atomicalsPayload.get())
    //console.log("cbor", atomicalsPayload.cbor().toString('hex'))
    //console.log("hashscript", hashscript.toString('hex'))
    //console.log("scriptP2TR", scriptP2TR.output?.toString('hex'))
    return {
        scriptP2TR,
        hashLockP2TR,
        hashscript
