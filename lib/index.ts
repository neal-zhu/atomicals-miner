...
import { MintInteractiveContainerDitemCommand } from "./commands/mint-container-ditem-command";
...
export class Atomicals implements APIInterface {
  ...
  async mintBatchContainerItemInteractive(options: BaseRequestOptions, container: string, itemNum: number, manifestPath: string, address: string, WIF: string, owner: IWalletRecord): Promise<CommandResultInterface> {
    try {
      await this.electrumApi.open();
      const command: CommandInterface = new MintInteractiveContainerDitemCommand(this.electrumApi, options, container, itemNum, manifestPath, address, WIF);
      return await command.run();
    } catch (error: any) {
      return {
        success: false,
